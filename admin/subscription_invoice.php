...
$stmt->execute([$id]);
$row = $stmt->fetch();
if(!$row) exit('اشتراك غير موجود');
$platform_name = "منصة فيزيتا سينا ديزاد";

// تحديد نوع المستخدم
$is_doctor = ($row['role'] === 'doctor');
$is_patient = ($row['role'] === 'patient');

// حساب الميزات لكل نوع
...
.platform-name { color:#0db98d; font-size:2rem; font-weight:bold; letter-spacing:1px; }
.invoice-title { font-size:1.3rem; color:#222; font-weight:700; margin-bottom:8px; }
...
<div class="invoice-box">
    ...
    <div class="header">
        <img src="<?=$is_doctor ? '../assets/img/LOGO.svg' : '../assets/img/LOGO.svg'?>" class="logo" alt="Logo" onerror="this.style.display='none'">
        <div class="platform-name"><?=$platform_name?></div>
        <div class="invoice-title">فاتورة اشتراك <?= $is_doctor ? 'طبيب' : 'مريض' ?></div>
    </div>
    <div class="info-row"><span class="info-label"><?= $is_doctor ? 'اسم الطبيب:' : 'اسم المريض:' ?></span> <span class="info-value"><?=htmlspecialchars($row['name'])?></span></div>
    <?php if($row['email']): ?>
